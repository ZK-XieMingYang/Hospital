...
public class MedicineInstockServiceImpl  implements IMedicineInstockService {
    ...
    @Override 
    public List findAll() {
        String sql="SELECT * from medicine_purchase_info mpi LEFT JOIN base_manufacturer bm ON" +
                " mpi.MAN_CODE=bm.man_Code\n" +
                "\t\t\t\t\t JOIN medicine_code mc ON mpi.MEDICINE_CODEID=mc.code_id WHERE mpi.STATUS=2";
